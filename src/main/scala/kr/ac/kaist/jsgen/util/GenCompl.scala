...
object GenCompl {
  // generate completion file jsgen-auto-completion
  def run: Unit = dumpFile(content, path)

  // commands
  val commands = JSGen.commands

  // phases
  val phases = JSGen.phases

  // global options
  val options = JSGen.options
  ...
  def content: String = {
    ...
    app >> "fi" >> LINE_SEP
    app >> LINE_SEP
    app >> "# completion for jsgen" >> LINE_SEP
    app >> "_jsgen_completions() {" >> LINE_SEP
    ...
    app >> "complete -o filenames -o bashdefault -F _jsgen_completions jsgen" >> LINE_SEP

    app.toString
  }

  // path of completion file
  val path = s"$BASE_DIR/jsgen-auto-completion"
