package kr.ac.kaist.jsgen.util

import scala.util.parsing.combinator._
import scala.io.Source
import kr.ac.kaist.jsgen.{ JSGen, Command, JSGenConfig }
import kr.ac.kaist.jsgen.phase.{ PhaseOption, Config }
...
class ArgParser(cmd: Command[_], jsgenConfig: JSGenConfig) extends RegexParsers {
  var ruleList: List[Parser[Unit]] = Nil

  var optNameSet: Set[String] = Set()

  addRule(jsgenConfig, "", JSGen.options)
  ...
  def apply(args: List[String]): Unit = {
          ...
          case Right(json) => json match {
            case obj if json.isObject => obj.asObject.get.toList.foreach {
              case (phase, value) if value.isObject => {
                if (JSGen.phases.map(_.name).contains(phase))
    ...
    lazy val fileName: Parser[Unit] = str ^^ {
      s => jsgenConfig.args = s :: jsgenConfig.args
    }
    ...
    jsgenConfig.args = jsgenConfig.args.reverse
  }
}
