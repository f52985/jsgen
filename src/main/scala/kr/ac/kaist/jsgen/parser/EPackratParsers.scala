package kr.ac.kaist.jsgen.parser
...
trait EPackratParsers extends Parsers {
  ...
  case class EPos(underlying: Position, emptyMatched: Boolean) extends Position {
    ...
    protected def lineContents =
      underlying.longString.substring(0, underlying.longString.length - column - 1)
    override def <(that: Position): Boolean = {
      this.line < that.line ||
        this.line == that.line && this.column < that.column ||
        this.line == that.line && this.column == that.column &&
        that.isInstanceOf[EPos] && !this.emptyMatched && that.asInstanceOf[EPos].emptyMatched
    }
  }

  class EPackratReader[+T <: Elem](underlying: Reader[T]) extends Reader[T] { outer =>
    val container: Container = Container()
    val rev: List[Elem] = Nil

    private[EPackratParsers] val cache = mutable.HashMap.empty[(Parser[_], Position), MemoEntry[_]]
    private[EPackratParsers] def getFromCache[T](p: Parser[T]): Option[MemoEntry[T]] = {
      cache.get((p, pos)).asInstanceOf[Option[MemoEntry[T]]]
    }
    private[EPackratParsers] def updateCacheAndGet[T](p: Parser[T], w: MemoEntry[T]): MemoEntry[T] = {
      cache.put((p, pos), w)
      w
    }
    private[EPackratParsers] val recursionHeads: mutable.HashMap[Position, Head] = mutable.HashMap.empty
    ...
    def rest: Reader[T] = new EPackratReader(underlying.rest) {
      ...
      override private[EPackratParsers] val recursionHeads = outer.recursionHeads
  ...
  private def recall(p: super.Parser[_], in: EPackratReader[Elem]): Option[MemoEntry[_]] = {
  ...
  private def setupLR(p: Parser[_], in: EPackratReader[_], recDetect: LR): Unit = {
  ...
  private def lrAnswer[T](p: Parser[T], in: EPackratReader[Elem], growable: LR): ParseResult[T] = growable match {
  ...
  def memo[T](p: super.Parser[T]): EPackratParser[T] = {
    new EPackratParser[T] {
      def apply(in: Input) = {
  ...
  private def grow[T](p: super.Parser[T], rest: EPackratReader[Elem], head: Head): ParseResult[T] = {
