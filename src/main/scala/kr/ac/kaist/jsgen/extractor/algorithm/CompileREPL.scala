...
import kr.ac.kaist.jsgen.extractor.algorithm.{ TokenParser, Compiler }
import kr.ac.kaist.jsgen.spec.JsonProtocol._
import kr.ac.kaist.jsgen.spec.algorithm.token.Token
...
object CompileREPL {
  def run(version: String, secIds: Map[String, String])(
    ...
    def prompt: String = LINE_SEP + s"${CYAN}jsgen>${RESET} "
    ...
    def parse(raw: Boolean, words: List[String]): Unit = {
      ...
      if (result.successful) {
        val resultStr = result.get.toString(line = true, asite = true)
