package kr.ac.kaist.jsgen.js.ast

import kr.ac.kaist.jsgen.ir._
import kr.ac.kaist.jsgen.util.Span
import kr.ac.kaist.jsgen.util.Useful._
import io.circe._, io.circe.syntax._

trait PropertyDefinitionList extends AST { val kind: String = "PropertyDefinitionList" }

object PropertyDefinitionList {
  def apply(data: Json): PropertyDefinitionList = AST(data) match {
    case Some(compressed) => PropertyDefinitionList(compressed)
    case None => error("invalid AST data: $data")
  }
  def apply(data: AST.Compressed): PropertyDefinitionList = {
    val AST.NormalCompressed(idx, subs, params, span) = data
    idx match {
      case 0 =>
        val x0 = subs(0).map(PropertyDefinition(_)).get
        PropertyDefinitionList0(x0, params, span)
      case 1 =>
        val x0 = subs(0).map(PropertyDefinitionList(_)).get
        val x1 = subs(1).map(PropertyDefinition(_)).get
        PropertyDefinitionList1(x0, x1, params, span)
    }
  }
}

case class PropertyDefinitionList0(x0: PropertyDefinition, parserParams: List[Boolean], span: Span) extends PropertyDefinitionList {
  x0.parent = Some(this)
  def idx: Int = 0
  def k: Int = d(x0, 0)
  def fullList: List[(String, PureValue)] = l("PropertyDefinition", x0, Nil).reverse
  def maxK: Int = 0
  override def toString: String = {
    s"$x0"
  }
}

case class PropertyDefinitionList1(x0: PropertyDefinitionList, x2: PropertyDefinition, parserParams: List[Boolean], span: Span) extends PropertyDefinitionList {
  x0.parent = Some(this)
  x2.parent = Some(this)
  def idx: Int = 1
  def k: Int = d(x2, d(x0, 0))
  def fullList: List[(String, PureValue)] = l("PropertyDefinition", x2, l("PropertyDefinitionList", x0, Nil)).reverse
  def maxK: Int = 0
  override def toString: String = {
    s"$x0 , $x2"
  }
}
