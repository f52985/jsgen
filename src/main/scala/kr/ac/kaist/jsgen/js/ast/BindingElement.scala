...
case class BindingElement0(x0: SingleNameBinding, parserParams: List[Boolean], span: Span) extends BindingElement {
  ...
  def idx: Int = 0
  def k: Int = d(x0, 0)
  def fullList: List[(String, PureValue)] = l("SingleNameBinding", x0, Nil).reverse
  def maxK: Int = 0
...
case class BindingElement1(x0: BindingPattern, x1: Option[Initializer], parserParams: List[Boolean], span: Span) extends BindingElement {
  ...
  def fullList: List[(String, PureValue)] = l("Option[Initializer]", x1, l("BindingPattern", x0, Nil)).reverse
  def maxK: Int = 1
  override def toString: String = {
    s"$x0 ${x1.getOrElse("")}"
  }
