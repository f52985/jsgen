...
object TypeModel {
  ...
  lazy val infos: List[Info] = List(
    ...
    I("PropertyDescriptor", Map(
      "Value" -> AbsType(ESValueT, AAbsent),
      "Writable" -> AbsType(BoolT, AAbsent),
      "Get" -> AbsType(NameT("FunctionObject"), AUndef, AAbsent),
      "Set" -> AbsType(NameT("FunctionObject"), AUndef, AAbsent),
      "Enumerable" -> AbsType(BoolT, AAbsent),
      "Configurable" -> AbsType(BoolT, AAbsent),
    )),

    // objects
    I("Object", Map(
      "SubMap" -> MapT(NameT("PropertyDescriptor")),
      "Prototype" -> AbsType(NameT("Object"), ANull),
      "Extensible" -> BoolT,
    ...
    I("ArrayBufferObject", parent = "Object", Map(
      "ArrayBufferData" -> AbsType(NameT("DataBlock"), ANull),
      "ArrayBufferByteLength" -> NumT,
      "ArrayBufferDetachKey" -> AUndef,
    )),
    ...
    I("GeneratorInstance", parent = "OrdinaryObject", Map(
      "GeneratorState" -> AbsType(AUndef, SUSPENDED_START, SUSPENDED_YIELD, EXECUTING, COMPLETED),
      "GeneratorContext" -> NameT("ExecutionContext"),
      "GeneratorBrand" -> EMPTY,
    )),
    I("AsyncGeneratorInstance", parent = "OrdinaryObject", Map(
      "AsyncGeneratorState" -> AbsType(AUndef, SUSPENDED_START, SUSPENDED_YIELD, EXECUTING, AWAITING_RETURN, COMPLETED),
    ...
    I("PromiseReactionRecord", Map(
      "Capability" -> AbsType(NameT("PromiseCapabilityRecord"), AUndef),
    ...
    I("EnvironmentRecord", parent = "LexicalEnvironment", Map(
      "OuterEnv" -> AbsType(NameT("EnvironmentRecord"), ANull),
    ...
    I("FunctionEnvironmentRecord", parent = "DeclarativeEnvironmentRecord", Map(
      ...
      "NewTarget" -> AbsType(NameT("Object"), AUndef),
    ...
    I("ExecutionContext", Map(
      "Function" -> AbsType(NameT("FunctionObject"), ANull),
      "Realm" -> NameT("RealmRecord"),
      "ScriptOrModule" -> AbsType(NameT("ScriptRecord"), NameT("ModuleRecord")),
    ...
    I("CandidateExecutionRecord", Map(
      ...
      "ChosenValues" -> NilT,
      "AgentOrder" -> AUndef,
      "ReadsBytesFrom" -> AUndef,
      "ReadsFrom" -> AUndef,
      "HostSynchronizesWith" -> AUndef,
      "SynchronizesWith" -> AUndef,
      "HappensBefore" -> AUndef,
    )),

    // script records
    I("ScriptRecord", Map(
      "Realm" -> AbsType(NameT("RealmRecord"), AUndef),
      "Environment" -> AbsType(NameT("EnvironmentRecord"), AUndef),
    ...
    I("ModuleRecord", Map(
      "Realm" -> AbsType(NameT("RealmRecord"), AUndef),
      "Environment" -> AbsType(NameT("ModuleEnvironmentRecord"), AUndef),
      "Namespace" -> AbsType(NameT("Object"), AUndef),
      "HostDefined" -> AUndef,
    ...
    I("CyclicModuleRecord", parent = "ModuleRecord", Map(
      ...
      "EvaluationError" -> AbsType(AbruptT, AUndef),
      "DFSIndex" -> AbsType(NumT, AUndef),
      "DFSAncestorIndex" -> AbsType(NumT, AUndef),
    ...
    I("ExportEntryRecord", Map(
      "ExportName" -> AbsType(StrT, ANull),
      "ModuleRequest" -> AbsType(StrT, ANull),
      "ImportName" -> AbsType(StrT, ANull),
      "LocalName" -> AbsType(StrT, ANull),
    )),
